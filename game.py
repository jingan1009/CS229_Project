...
class Game:
    ...
    def run(self):
        ...
        if self.agents[0].is_test and self.state.isWin():
            TESTWINS += 1

            TESTREWARD += self.state.getScore()
            # print(TESTWINS)
            # print(TESTREWARD)
        # else: 
        #     if self.state.isWin():
        ...
        if self.agents[0].testcounter==50:
            print('enter')
            avg_win = (1.0*TESTWINS)/50.0
            if TESTWINS>0:
                avg_win_score = (1.0*TESTREWARD)/TESTWINS
            else:
                avg_win_score = 0
            # print(avg_win)
            # print(avg_win_score)
            results_file = open('./testScores/'+'Fri_15_16_40_'+'smallGrid'+'.log','a')
            results_file.write("# num_test: 100 | Average test winrate: %12f | Average test win-score: %12f \n" %(avg_win, avg_win_score))
            TESTWINS = 0
            TESTREWARD = 0
